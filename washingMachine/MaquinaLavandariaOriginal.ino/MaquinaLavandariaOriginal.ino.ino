...
void lavagem(int tempo, int minDiferenca, int secDiferenca) {
  ...
  int pausaMinutos = 0, diferenca = 0;//,pausaSegundos =0, difSeconds =0, difMints = 0, minFim=0, secFim=0;
  long cornometro = 0.0;
  int minutes = 0;
  ...
  do {
    ...
    cornometro = millis();
    seconds = ((((cornometro % day) % hour) % minute) / second) - secDiferenca  ;
    minutes = (((cornometro % day) % hour) / minute) - minDiferenca ;
    ...
    /**if (digitalRead(11) == 1) {
      ...
      delay(200);
        ...
        do {
          ...
          Serial.print(":");
          Serial.println(pausaSegundos);
          
        if (digitalRead(11) == 1) {

          digitalWrite(agua, 0);
          diferenca = minutes - pausaMinutos;
          break;
        }
      } while (digitalRead(11)!=1);  secFim = seconds; minFim=minutes; difMints = minFim - pausaMinutos; difSeconds = secFim - pausaSegundos;
     
        delay(200);
    }
    Serial.print("Diferenca");
    Serial.print(difMints);
    Serial.print(":");
    Serial.println(difSeconds);*/
  }
  while (minutes < tempoTotalEstatico + diferenca);
  lcd.setCursor(0, 0);
  lcd.print("                ");
